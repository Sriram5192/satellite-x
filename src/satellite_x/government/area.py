...
from .models import AreaSummary, FieldGovernmentRecord


class AreaAggregator:
    """Privacy-safe village, mandal, or district aggregation."""
    ...
    def aggregate(
        ...
        scope_code: str,
        fields: list[FieldGovernmentRecord],
    ) -> AreaSummary:
        attribute = f"{scope_level}_code"
        selected = [item for item in fields if getattr(item, attribute) == scope_code]
        ...
        crop: defaultdict[str, float] = defaultdict(float)
        verdict: defaultdict[str, float] = defaultdict(float)
